...
static sb_u64 tail_parse_n_or_die(const char *argv0, const char *s) {
	...
	if (sb_parse_u64_dec(s, &n) != 0) {
		sb_die_usage(argv0, "tail [-n N] [-c N] [-f] [FILE...]");
	}
	return n;
}
...
static void tail_follow_fd(const char *argv0, sb_i32 fd) {
	sb_u8 buf[32768];
	...
	for (;;) {
		...
		if (r < 0) {
			if ((sb_u64)(-r) == (sb_u64)SB_EINTR) continue;
		...
		struct sb_stat st;
		sb_i64 fr = sb_sys_fstat(fd, &st);
		if (fr < 0) sb_die_errno(argv0, "fstat", fr);
		sb_i64 cur = sb_sys_lseek(fd, 0, SB_SEEK_CUR);
		if (cur < 0) sb_die_errno(argv0, "lseek", cur);
		if (st.st_size < cur) {
			sb_i64 sr = sb_sys_lseek(fd, 0, SB_SEEK_SET);
...
static int tail_path(const char *argv0, const char *path, int bytes_mode, int follow, sb_u64 n) {
	if (sb_streq(path, "-")) {
		// Minimal policy: follow mode requires a seekable file.
		if (follow) sb_die_usage(argv0, "tail [-n N] [-c N] [-f] [FILE...]");
	...
	if (follow) {
		...
		sb_i64 sr = sb_sys_lseek((sb_i32)fd, 0, SB_SEEK_END);
		if (sr < 0) sb_die_errno(argv0, "lseek", sr);
		tail_follow_fd(argv0, (sb_i32)fd);
	}
	(void)sb_sys_close((sb_i32)fd);
	return 0;
}
